...
export default class SoundBrowser {
    ...
    render() {
        ...
        drawText(`Sounds: (${this.currentlyPlayingSoundId}/${SOUNDS.length})`, [0.5, 0.04], 4, [255, 255, 255, 200], 0.5, true);
        const previousSound = (SOUNDS.length >= this.currentlyPlayingSoundId - 1) ? SOUNDS[this.currentlyPlayingSoundId - 1] : false;
        if (previousSound) {
        ...
        if (nextSound) {
            drawText(`~y~${nextSound.soundName} ~w~| ~b~${nextSound.soundSetName}`, [0.5, 0.16], 4, [255, 255, 255, 100], 0.45, true);
        }

        if (game.isControlPressed(0, controlIds.F)) { // F
            drawText(`~b~LEWO / PRAWO ~w~zmian dzwieku~n~`, [0.5, 0.75], 4, [255, 255, 255, 200], 0.5, true);
            drawText(`~b~GORA / DOL ~w~zmiana dzwieku o +10/-10~n~`, [0.5, 0.78], 4, [255, 255, 255, 200], 0.5, true);
        } else {
    ...
    play(soundId) {
        ...
        game.playSoundFrontend(-1, sound.soundName, sound.soundSetName, true);
        this.currentlyPlayingSoundId = sound.id;
        alt.log(`Sound id = ${this.currentlyPlayingSoundId}`);
    }
    ...
    nextSound(offset = 1) {
        try {
            ...
            if (nextSound) {
                ...
                alt.log(`Did not found sound with id = ${this.currentlyPlayingSoundId + offset}`);
            }
        } catch (error) {
            alt.log(`Error while playing next sound ${error}`);
        }
    }
    ...
    previousSound(offset = 1) {
        try {
            ...
            alt.log(`Error while playing previous sound ${error}`);
        }
