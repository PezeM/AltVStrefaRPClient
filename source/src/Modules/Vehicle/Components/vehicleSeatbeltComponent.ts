...
class VehicleSeatbeltComponent extends VehicleComponent {
  ...
  onUpdateInVehicle(localPlayer: alt.Player) {
    ...
    if (!this.isSeatbeltOn) {
      ...
      const isVehicleMovingForward =
        game.getEntitySpeedVector(playerVehicle.scriptID, true).y > 1.0;
      const vehicleAcceleration = this.calculateVehicleAcceleration(
        previousSpeed
      );
      if (
        this.checkConditions(
          isVehicleMovingForward,
          previousSpeed,
          vehicleAcceleration
        )
      ) {
        alt.log(
          `Should ragdoll vehicle accelaration was ${vehicleAcceleration}`
        );
  ...
  setRagdollEffect(localPlayer: alt.Player) {
    ...
    game.setEntityVelocity(
      localPlayer.scriptID,
      this.previousVelocity.x,
      this.previousVelocity.y,
      this.previousVelocity.z
    );
    alt.setTimeout(() => {
      game.setPedToRagdoll(
        localPlayer.scriptID,
        1000,
        1000,
        0,
  ...
  checkConditions(
    isVehicleMovingForward: boolean,
    previousSpeed: number,
    vehicleAcceleration: number
  ) {
    return (
      isVehicleMovingForward &&
      previousSpeed > SEATBELT_EJECT_SPEED / 3.6 &&
      vehicleAcceleration > SEATBELT_EJECT_ACCELERATION
    );
  }
  ...
  toggleSeatbelt(localPlayer: alt.Player) {
    ...
    const correctVehicleClass = this.correctVehicleClass(
      localPlayer.vehicle as alt.Vehicle
    );
    if (!correctVehicleClass) return;
    this.isSeatbeltOn = !this.isSeatbeltOn;
    this.isSeatbeltOn
      ? sounds.playCefSound("buckle.webm", 0.5, false)
      : sounds.playCefSound("unbuckle.webm", 0.5, false);
    alt.setTimeout(() => {
      const text = this.isSeatbeltOn
        ? "Zapięto pasy bezpieczeństwa."
        : "Odpięto pasy bezpieczeństwa";
  ...
  correctVehicleClass(vehicle: alt.Vehicle) {
    ...
    if (vehicleClass === 8 || vehicleClass === 13) return false;
    // No seatbelt on motorcycles/bikes
    else return true;
  }
}
