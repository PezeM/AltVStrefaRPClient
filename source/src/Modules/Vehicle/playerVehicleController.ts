...
class PlayerVehicleController {
  ...
  vehicleLeaveEvent: TypedEvent<VehicleLeaveEventArgs> = new TypedEvent<
    VehicleLeaveEventArgs
  >();
  vehicleEnterEvent: TypedEvent<VehicleEnterEventArgs> = new TypedEvent<
    VehicleEnterEventArgs
  >();
  vehicleSeatChangeEvent: TypedEvent<
    VehicleSeatChangeEventArgs
  > = new TypedEvent<VehicleSeatChangeEventArgs>();
  ...
  private tick() {
    ...
    if (playerVehicle !== this.currentVehicle) {
      if (playerVehicle === null) {
        ...
        this.vehicleLeave(
          this.previousVehicle as alt.Vehicle,
          this.previousSeat
        );
        return;
      } else {
        ...
        for (
          let i = -1;
          i < game.getVehicleMaxNumberOfPassengers(playerVehicle.scriptID);
          i++
        ) {
          const pedAtSeat = game.getPedInVehicleSeat(
            playerVehicle.scriptID,
            i,
            0
          );
