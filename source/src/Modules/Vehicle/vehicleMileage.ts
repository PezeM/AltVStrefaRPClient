...
class VehicleMileage {
  ...
  private tick() {
    ...
    // drawText(
    //   `Vehicle mileage: ${playerVehicle.mileage.toFixed(2)}`,
    //   [0.2, 0.2],
    //   4,
    //   [255, 255, 255, 255],
    //   0.6,
    ...
    // );
    // drawText(
    //   `Calculated dist: ${this.calculatedDist.toFixed(2)}`,
    //   [0.2, 0.25],
    //   4,
    //   [255, 255, 255, 255],
    //   0.6,
  ...
  private onVehicleEnter = (args: VehicleEnterEventArgs) => {
    playerVehicleController.vehicleSeatChangeEvent.on(
      this.onVehicleSeatChanged
    );
  ...
  private onVehicleLeave = (args: VehicleLeaveEventArgs) => {
    ...
    playerVehicleController.vehicleSeatChangeEvent.off(
      this.onVehicleSeatChanged
    );
  };
  ...
  private startMileageCounter(playerVehicle: alt.Vehicle) {
    ...
    if (!playerVehicle) {
      alt.logError(
        "Coulnd start mileage counter, because player vehicle was not found."
      );
      return;
    }
