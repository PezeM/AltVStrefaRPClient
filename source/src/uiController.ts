  ...
  switch (key) {
    case controlsIds.Alt:
      if (
        ...
        new Date().getTime() - lastKeyPressedTime < 300
      )
        return;
...
alt.setInterval(() => {
  if (
    localPlayer.vehicle == null &&
    !game.isPlayerDead(localPlayer.scriptID) &&
    !mainUi.viewOpened
  ) {
    if (!circleMenu.isMenuOpened) raycast.poitingAt(4);
  } else {
...
alt.setInterval(() => {
  ...
  if (zoneNames.realZoneName && zoneNames.streetName && zoneNames.minimap) {
    drawText(
      zoneNames.streetName,
      [zoneNames.minimap.rightX, zoneNames.minimap.bottomY - 0.065],
      4,
      [255, 255, 255, 255],
      0.5,
      true,
    ...
    drawText(
      zoneNames.realZoneName,
      [zoneNames.minimap.rightX, zoneNames.minimap.bottomY - 0.035],
      4,
      [255, 255, 255, 255],
      0.55,
      true,
...
alt.onServer(
  "showConfirmModal",
  (title: string, message: string, type: number, args: any) => {
    alt.log(`Args: ${JSON.stringify(args)}`);
    switch (type) {
      case 1: // Business invite
        alt.showCursor(true);
        mainUi.showConfirmModal(
          title,
          message,
      ...
      case 2: // Fraction invite
        ...
        mainUi.showConfirmModal(
          title,
          message,
          "acceptFractionInvite",
          "cancelFractionInvite",
          args
        );
        break;
      default:
